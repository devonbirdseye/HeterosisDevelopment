...
```
```{r plastid ribosome heatmap - section totals}
colnames(tmt4.n)
tmt4.n.pRibo.m <- as.matrix(tmt4.n[,25:28])
rownames(tmt4.n.pRibo.m) <- tmt4.n$Gene
heatmap(tmt4.n.pRibo.m, cexCol = .8, cexRow = .4)
```
...
```
```{r photosynthesis-associated heatmap - section totals}
tmt4.n.PhAGs.m <- as.matrix(tmt4.n[tmt4.n$Gene %in% c(GOIs$PhANGs, GOIs$PhAPGs),25:28])
rownames(tmt4.n.PhAGs.m) <- tmt4.n$Gene[tmt4.n$Gene %in% c(GOIs$PhANGs, GOIs$PhAPGs)]
